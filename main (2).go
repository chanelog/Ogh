...
func loadUsers() UserDatabase {
	data, err := os.ReadFile("/etc/zivpn/users.json")
	if err != nil {
		return UserDatabase{Users: []User{}}
	}
	var db UserDatabase
...
func saveUsers(db UserDatabase) error {
	...
	return os.WriteFile("/etc/zivpn/users.json", data, 0600)
}
...
func install() {
	...
	// 3. DB user
	if _, err := os.Stat("/etc/zivpn/users.json"); os.IsNotExist(err) {
		saveUsers(UserDatabase{Users: []User{}})
	}
