...
_bak_collect() {
    ...
    # Juga backup binary jika ada
    [[ -f "$BIN" ]] && BAK_FILES+=("$BIN")
}

# Tampilkan daftar backup, echo jumlah file ke stdout baris terakhir
_bak_list() {
    mkdir -p "$BAKDIR"
    # Baca ke array untuk menghindari glob expand ke literal '*'
    local -a files=()
    ...
    if [[ $cnt -eq 0 ]]; then
        echo ""
        warn "Belum ada file backup di ${W}${BAKDIR}${NC}"
        echo "0"; return
    fi
    ...
    for f in "${files[@]}"; do
        ...
        # simpan ke array global supaya bisa diakses by nomor
        BAK_FILE_LIST[$i]="$f"
...
_bak_do_create() {
    ...
    if [[ ! -w "$BAKDIR" ]]; then
        err "Folder backup tidak bisa ditulis: ${W}$BAKDIR${NC}"
        return 1
    fi

    inf "Membuat backup → ${W}$bfile${NC}"
    # Gunakan -P (absolute path) agar restore bisa tepat ke lokasi asli
...
# Auto-cleanup backup lama, sisakan $1 terbaru
_bak_cleanup_old() {
    local keep="${1:-10}"
    local -a files=()
    while IFS= read -r -d '' f; do
...
svc_backup() {
    ...
    while true; do
        ...
        _btn "  ${DIM}Total   : ${Y}${total_bak} file${NC}  •  ${DIM}Size: ${A3}${bak_size:-0}${NC}"
        _sep
        _btn "  ${A2}[1]${NC}  📦  Buat Backup Sekarang"
        _sep
        _btn "  ${A2}[2]${NC}  📋  Lihat Daftar Backup"
        _sep
        _btn "  ${A2}[3]${NC}  📤  Kirim Backup ke Telegram"
        _sep
        _btn "  ${A2}[4]${NC}  🗑️   Hapus Backup"
        _sep
        ...
        1)
            show_header
            _top; _btn "  ${IT}${AL}📦  BUAT BACKUP SEKARANG${NC}"; _bot; echo ""
            local bfile="${BAKDIR}/oghziv-backup-$(date +%Y%m%d-%H%M%S).tar.gz"
            if _bak_do_create "$bfile"; then
                # Auto-cleanup simpan 10 terbaru
                _bak_cleanup_old 10
                # Notif Telegram
                local sz; sz=$(du -sh "$bfile" 2>/dev/null | cut -f1)
                _tg_send "💾 <b>Backup Berhasil</b>
📁 File: <code>$(basename "$bfile")</code>
📦 Ukuran: $sz
🕐 Waktu: $(date '+%d/%m/%Y %H:%M:%S')"
        ...
        2)
            ...
            local cnt; cnt=$(_bak_list)
            [[ "$cnt" == "0" ]] && echo -e "\n  ${DIM}Buat backup dulu dengan pilihan [1]${NC}"
            pause
            ;;

        3)
            show_header
            _top; _btn "  ${IT}${AL}📤  KIRIM BACKUP KE TELEGRAM${NC}"; _bot; echo ""
            if [[ ! -f "$BOTF" ]]; then
                err "Bot Telegram belum dikonfigurasi!"; pause; continue
            fi
            source "$BOTF" 2>/dev/null
            ...
            [[ ! "$bno" =~ ^[0-9]+$ || $bno -lt 1 || $bno -gt $cnt ]] && { err "Nomor tidak valid!"; pause; continue; }
            local bpath="${BAK_FILE_LIST[$bno]}"
            [[ ! -f "$bpath" ]] && { err "File tidak ditemukan!"; pause; continue; }
            local sz; sz=$(du -sh "$bpath" | cut -f1)
            inf "Mengirim ke Telegram... (bisa memakan waktu)"
            local rsp
            rsp=$(curl -s --max-time 120 -X POST \
                "https://api.telegram.org/bot${BOT_TOKEN}/sendDocument" \
                -F "chat_id=${CHAT_ID}" \
                -F "document=@${bpath}" \
                -F "caption=💾 Backup OGH-ZIV%0A📁 $(basename "$bpath")%0A📦 $sz%0A🕐 $(date '+%d/%m/%Y %H:%M')" \
                2>/dev/null)
            echo "$rsp" | grep -q '"ok":true' \
                && ok "Backup berhasil dikirim ke Telegram!" \
                || err "Gagal kirim! Cek token/Chat ID atau ukuran file (max 50MB)."
            pause
            ;;

        4)
            show_header
            _top; _btn "  ${IT}${AL}🗑️   HAPUS BACKUP${NC}"; _bot
            local cnt; cnt=$(_bak_list)
            [[ "$cnt" == "0" ]] && { pause; continue; }
            ...
            if [[ "${bno,,}" == "all" ]]; then
                ...
                [[ "$cf" == [yY] ]] && \
                    { rm -f "${BAKDIR}"/oghziv-backup-*.tar.gz; ok "Semua backup dihapus!"; } || \
                    inf "Dibatalkan."
...
svc_restore() {
    ...
    while true; do
        ...
        _sep
        _btn "  ${A2}[1]${NC}  📋  Restore dari Daftar Backup"
        _sep
        _btn "  ${A2}[2]${NC}  📁  Restore dari Path Manual"
        _sep
        _btn "  ${A2}[3]${NC}  🔍  Verifikasi / Lihat Isi Backup"
        _sep
        _btn "  ${LR}[0]${NC}  ◀   Kembali"
        _bot; echo ""
        echo -ne "  ${A1}›${NC} "; read -r ch
        ...
        1)
            show_header
            _top; _btn "  ${IT}${AL}📋  PILIH BACKUP UNTUK RESTORE${NC}"; _bot
            local cnt; cnt=$(_bak_list)
            if [[ "$cnt" == "0" ]]; then
                echo -e "\n  ${DIM}Gunakan pilihan [2] jika file backup ada di path lain.${NC}"
                pause; continue
            fi
            echo -ne "  ${A3}Nomor backup yang direstore${NC} [1-$cnt]: "; read -r bno
            [[ ! "$bno" =~ ^[0-9]+$ || $bno -lt 1 || $bno -gt $cnt ]] && { err "Nomor tidak valid!"; pause; continue; }
            _bak_do_restore "${BAK_FILE_LIST[$bno]}"
            ;;
        ...
        2)
            ...
            bpath="${bpath//\'/}"   # strip single quote jika ada
            bpath="${bpath// /}"    # strip spasi jika ada
            if [[ ! -f "$bpath" ]]; then
                err "File tidak ditemukan: ${W}$bpath${NC}"
                echo ""
                inf "Contoh path: ${DIM}/root/oghziv-backups/oghziv-backup-20260327-123456.tar.gz${NC}"
                pause; continue
            fi
...
_bak_do_restore() {
    ...
    warn "Backup otomatis akan dibuat terlebih dahulu."
    echo ""
    ...
    if tar -xPf "$bpath" 2>/tmp/oghziv_rst_err; then
        ...
        if is_up; then
            ...
            warn "ZiVPN belum jalan — coba restart manual: ${Y}systemctl start zivpn${NC}"
        fi
        # Notif Telegram
