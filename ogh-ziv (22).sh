...
_bak_restore_forward_detect() {
    ...
    echo -e "  ${A1}║${NC}  ${A2}3.${NC} ${LG}Forward${NC} file backup (.tar.gz) ke bot:     ${A1}║${NC}"
    printf  "  ${A1}║${NC}     ${Y}%-50s${NC}  ${A1}║${NC}\n" "@${BOT_NAME:-your_bot}"
    echo -e "  ${A1}║${NC}  ${A2}4.${NC} Tekan Enter di sini setelah forward           ${A1}║${NC}"
    echo -e "  ${A1}╚══════════════════════════════════════════════════════╝${NC}"
    echo ""
    echo -ne "  ${A3}Sudah forward file ke bot? Tekan Enter untuk deteksi${NC}..."
    read -r

    echo ""
    inf "Mendeteksi file yang baru di-forward..."
    echo ""

    # Ambil update terbaru dari bot
    local tmp_upd="/tmp/oghziv_fwd_$$.json"
    curl -s --max-time 30 \
        "https://api.telegram.org/bot${use_token}/getUpdates?limit=20&offset=-1" \
        -o "$tmp_upd" 2>/dev/null

    if [[ ! -s "$tmp_upd" ]]; then
        rm -f "$tmp_upd"
        err "Tidak ada response dari Telegram! Cek koneksi internet."; pause; return
    fi

    # Parse: cari file .tar.gz dari update paling baru
    local parsed
    parsed=$(python3 - "$use_chatid" "$tmp_upd" <<'PYEOF'
import sys, json
...
# Ambil semua update, tidak harus cocok chat_id (forward bisa dari chat berbeda)
for item in reversed(results):
    for msg_key in ("message", "channel_post", "edited_message"):
        msg = item.get(msg_key, {})
        if not msg:
        ...
        size_mb = round(fsize / 1048576, 2) if fsize else 0
        msg_chat_id = str(msg.get("chat", {}).get("id", "")).strip()
        docs.append(f"{fid}|{fname}|{size_mb}MB|{msg_chat_id}")
...
PYEOF
    ...
    if [[ -z "$parsed" ]]; then
        err "Tidak ada file .tar.gz terdeteksi dari update terbaru."
        echo ""
        inf "Pastikan kamu sudah forward file backup ke bot, lalu coba lagi."
        inf "Atau gunakan opsi [2] Input File ID manual."
        pause; return
    fi
    ...
    while IFS= read -r line; do
        [[ -z "$line" ]] && continue
        ((i++))
        local fid fname fsize fchat
        IFS='|' read -r fid fname fsize fchat <<< "$line"
        fwd_ids+=("$fid")
        fwd_names+=("$fname")
