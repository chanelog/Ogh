...
setup_awal(){
  ...
  chmod -R +x "$udp_file"
  buat_limitador
  cp "$(realpath "$0")" "$udp_file/UDPserver.sh" 2>/dev/null
  chmod +x "$udp_file/UDPserver.sh"
  printf '#!/bin/bash\nbash %s/UDPserver.sh "$@"\n' "$udp_file" > /usr/bin/udp
  chmod +x /usr/bin/udp
}
...
if [[ ! -f "$udp_file/UDPserver.sh" ]]; then
  setup_awal
  install_deps
  title "INSTALASI AWAL SELESAI"
  print_center -ama "Gunakan perintah:"
  print_center -verd "udp"
  print_center -ama "untuk membuka menu"
  msg -bar
  time_reboot 10
fi
