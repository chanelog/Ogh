...
_bak_tg_list() {
    ...
    source "$BOTF" 2>/dev/null
    # Gunakan variabel BOT_TOKEN & CHAT_ID saat ini (bisa sudah di-override oleh caller)
    ...
    # offset=-1 memastikan semua update lama juga bisa dibaca ulang
    local tmp_updates="/tmp/oghziv_tg_updates_$$.json"
    curl -s --max-time 30 \
        "https://api.telegram.org/bot${BOT_TOKEN}/getUpdates?limit=100&offset=-1" \
        -o "$tmp_updates" 2>/dev/null

    # Jika getUpdates kosong (bot sudah lama aktif, queue kosong), coba tanpa offset
    local update_count
    update_count=$(python3 -c \
        "import json,sys; d=json.load(open(sys.argv[1])); print(len(d.get('result',[])))" \
        "$tmp_updates" 2>/dev/null || echo 0)

    if [[ "$update_count" == "0" ]]; then
        # Fallback: coba tanpa offset
        curl -s --max-time 30 \
    ...
    # FIX: Bandingkan chat_id sebagai string setelah strip whitespace agar tidak mismatch
    local parsed
...
for item in reversed(results):
    # Cek di message biasa maupun channel_post
    for msg_key in ("message", "channel_post", "edited_message"):
        msg = item.get(msg_key, {})
        if not msg:
            continue
        ...
        size_mb = round(fsize / 1048576, 2) if fsize else 0
        docs.append(f"{fid}|{fname}|{size_mb}MB")
...
# FIX: Tidak re-source BOTF agar token override dari caller (_bak_restore_from_telegram) tetap berlaku
_bak_tg_download() {
    local file_id="$1" outfile="$2"
    # Gunakan BOT_TOKEN dari environment saat ini (bisa sudah di-override)
    # Jika belum ada, baru load dari BOTF
    if [[ -z "$BOT_TOKEN" ]]; then
        [[ ! -f "$BOTF" ]] && return 1
        source "$BOTF" 2>/dev/null
    fi
    [[ -z "$BOT_TOKEN" ]] && { err "BOT_TOKEN tidak tersedia!"; return 1; }
    ...
    if ! echo "$file_info" | grep -q '"ok":true'; then
        local tg_err; tg_err=$(echo "$file_info" | python3 -c \
            "import sys,json; d=json.load(sys.stdin); print(d.get('description','Unknown'))" 2>/dev/null)
        err "Gagal mendapatkan info file: ${LR}${tg_err:-cek token & file_id}${NC}"
        return 1
    fi
    ...
    if [[ -z "$file_path" ]]; then
        err "Gagal parse path file dari Telegram!"
        return 1
    fi
    ...
    local tmp_dl="${outfile}.tmp"
    if curl -s --max-time 300 -L "$dl_url" -o "$tmp_dl" 2>/dev/null; then
        if [[ -s "$tmp_dl" ]]; then
            mv "$tmp_dl" "$outfile"
            ...
            err "File kosong setelah download! Mungkin file_id sudah expired di Telegram."
            rm -f "$tmp_dl"; return 1
        fi
    else
        err "Download gagal! Cek koneksi VPS ke Telegram."
        rm -f "$tmp_dl"; return 1
    fi
}
...
#  Mendukung getUpdates + input file_id manual + forward-detect
# ════════════════════════════════════════════════════════════
_bak_restore_from_telegram() {
    ...
    echo -e "  ${A1}║${NC}  ${A2}[4]${NC}  📨  Forward file ke bot → deteksi otomatis ${A1}║${NC}"
    echo -e "  ${A1}╚══════════════════════════════════════════════════════╝${NC}"
    echo ""
    echo -e "  ${DIM}💡 Pindah VPS? Gunakan opsi [4] — forward file backup${NC}"
    echo -e "  ${DIM}   dari Telegram ke bot, lalu script deteksi otomatis.${NC}"
    echo ""
    echo -ne "  ${A3}Pilih metode${NC} [1-4]: "; read -r tch

    local use_token="$BOT_TOKEN"
    local use_chatid="$CHAT_ID"

    # Opsi 4: Forward file ke bot → tunggu → deteksi otomatis
    if [[ "$tch" == "4" ]]; then
        _bak_restore_forward_detect "$use_token" "$use_chatid"
        return
    fi
    ...
    if [[ "$tch" == "2" ]]; then
        ...
        echo -e "  ${DIM}1. Forward file backup ke bot kamu di Telegram${NC}"
        ...
        # Override token jika berbeda — gunakan export agar fungsi download tidak re-source BOTF
        local orig_token="$BOT_TOKEN"
        export BOT_TOKEN="$use_token"
        export CHAT_ID="$use_chatid"

        if _bak_tg_download "$manual_fid" "$dl_path"; then
            export BOT_TOKEN="$orig_token"
    ...
    # FIX: Export supaya subshell/fungsi yang dipanggil bisa baca nilai override
    local orig_token="$BOT_TOKEN"
    ...
    if ! _bak_tg_list; then
        export BOT_TOKEN="$orig_token"
        export CHAT_ID="$orig_chatid"
        echo ""
        inf "Tidak ditemukan otomatis. Coba cara lain:"
        echo -e "  ${DIM}• Pilih [4] untuk forward file backup ke bot (disarankan untuk VPS baru)${NC}"
    ...
    [[ ! "$bno" =~ ^[0-9]+$ || $bno -lt 1 || $bno -gt $TG_BAK_CNT ]] && {
        ...
        export BOT_TOKEN="$orig_token"; export CHAT_ID="$orig_chatid"
        pause; return; }
...
# ════════════════════════════════════════════════════════════
#  RESTORE: FORWARD FILE KE BOT → DETEKSI OTOMATIS
#  Solusi untuk VPS baru yang getUpdates-nya kosong
# ════════════════════════════════════════════════════════════
_bak_restore_forward_detect() {
    local use_token="$1" use_chatid="$2"
    show_header
    _top; _btn "  ${IT}${AL}📨  RESTORE VIA FORWARD FILE KE BOT${NC}"; _bot; echo ""

    echo -e "  ${A1}╔══════════════════════════════════════════════════════╗${NC}"
    echo -e "  ${A1}║${NC}  ${A2}LANGKAH:${NC}                                          ${A1}║${NC}"
    echo -e "  ${A1}║${NC}                                                      ${A1}║${NC}"
    echo -e "  ${A1}║${NC}  ${A2}1.${NC} Buka Telegram di HP/PC kamu                  ${A1}║${NC}"
    echo -e "  ${A1}║${NC}  ${A2}2.${NC} Cari chat/grup tempat backup disimpan         ${A1}║${NC}"
    echo -e "  ${A1}║${NC}  ${A2}3.${NC} ${LG}Kirim/Forward${NC} file (.tar.gz) ke bot:      ${A1}║${NC}"
    printf  "  ${A1}║${NC}     ${Y}%-50s${NC}  ${A1}║${NC}\n" "@${BOT_NAME:-your_bot}"
    echo -e "  ${A1}║${NC}  ${A2}4.${NC} Tekan Enter di sini setelah mengirim          ${A1}║${NC}"
    echo -e "  ${A1}╚══════════════════════════════════════════════════════╝${NC}"
    echo ""

    while true; do
        echo -ne "  ${A3}Sudah kirim file ke bot? Tekan Enter untuk deteksi${NC}..."
        read -r

        echo ""
        inf "Mendeteksi file... (membaca semua update pending bot)"
        echo ""

        local tmp_upd="/tmp/oghziv_fwd_$$.json"

        # FIX: offset=0 baca SEMUA update pending bot ini sejak awal.
        # offset=-1 hanya baca 1 update terakhir saja — kalau bot lama
        # sudah consume queue, hasilnya kosong. offset=0 tidak ada itu.
        curl -s --max-time 30 \
            "https://api.telegram.org/bot${use_token}/getUpdates?limit=100&offset=0" \
            -o "$tmp_upd" 2>/dev/null

        if [[ ! -s "$tmp_upd" ]]; then
            rm -f "$tmp_upd"
            err "Tidak ada response dari Telegram! Cek koneksi internet VPS."
            echo -ne "  ${A3}Coba lagi? [Y/n]${NC}: "; read -r ra
            [[ "${ra,,}" == "n" ]] && { inf "Gunakan opsi [2] Input File ID manual."; pause; return; }
            continue
        fi

        local parsed
        parsed=$(python3 - "$tmp_upd" <<'PYEOF'
import sys, json
tmp_file = sys.argv[1]
try:
    with open(tmp_file) as f:
...
PYEOF
        ...
        if [[ -z "$parsed" ]]; then
            err "Tidak ada file .tar.gz terdeteksi."
            echo ""
            echo -e "  ${DIM}Kemungkinan penyebab:${NC}"
            echo -e "  ${DIM}• File belum sampai — tunggu beberapa detik lalu coba lagi${NC}"
            echo -e "  ${DIM}• Pastikan file dikirim langsung ke bot (bukan grup lain)${NC}"
            echo -e "  ${DIM}• Nama file harus berakhiran .tar.gz${NC}"
            echo ""
            echo -ne "  ${A3}Coba lagi? [Y/n]${NC}: "; read -r retry_ans
            [[ "${retry_ans,,}" == "n" ]] && {
                inf "Gunakan opsi [2] Input File ID manual sebagai alternatif."
                pause; return
            }
            continue
        fi
    ...
    # Tampilkan file yang terdeteksi
    local i=0
    local -a fwd_ids=() fwd_names=()
    echo -e "  ${A1}╔═══╦════════════════════════════════════════╦════════╗${NC}"
    echo -e "  ${A1}║${NC} ${DIM}No${NC} ${A1}║${NC} ${DIM}Nama File                                ${A1}║${NC} ${DIM}Ukuran${NC} ${A1}║${NC}"
    echo -e "  ${A1}╠═══╬════════════════════════════════════════╬════════╣${NC}"
    while IFS= read -r line; do
        ...
        fwd_ids+=("$fid")
        fwd_names+=("$fname")
        printf "  ${A1}║${NC} ${A2}%2d${NC} ${A1}║${NC} ${W}%-40s${NC} ${A1}║${NC} ${Y}%6s${NC} ${A1}║${NC}\n" \
               "$i" "${fname:0:40}" "$fsize"
    done <<< "$parsed"
    echo -e "  ${A1}╚═══╩════════════════════════════════════════╩════════╝${NC}"
    echo ""

    echo -ne "  ${A3}Pilih nomor${NC} [1-$i]: "; read -r bno
    [[ ! "$bno" =~ ^[0-9]+$ || $bno -lt 1 || $bno -gt $i ]] && {
        err "Nomor tidak valid!"; pause; return; }

    local sel_id="${fwd_ids[$((bno-1))]}"
    local sel_name="${fwd_names[$((bno-1))]}"
    local dl_path="${BAKDIR}/${sel_name}"
    mkdir -p "$BAKDIR"
