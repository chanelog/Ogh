...
# Ambil daftar file backup dari riwayat pesan Telegram
# BOT_TOKEN & CHAT_ID harus sudah diset oleh pemanggil; hanya fallback ke BOTF jika kosong
_bak_tg_list() {
...
# Parameter opsional ke-3: override token (untuk restore dari VPS beda / token berbeda)
_bak_tg_download() {
    local file_id="$1" outfile="$2" override_token="${3:-}"

    # Tentukan token yang dipakai:
    # 1. Jika parameter ke-3 diberikan, pakai itu (restore lintas VPS / token lain)
    # 2. Jika BOT_TOKEN sudah diset oleh pemanggil, pakai itu
    # 3. Fallback: baca dari BOTF
    local dl_token="$override_token"
    if [[ -z "$dl_token" ]]; then
        # Pakai BOT_TOKEN yang sudah di-set sebelumnya oleh pemanggil (jangan source ulang!)
        dl_token="${BOT_TOKEN:-}"
    fi
    if [[ -z "$dl_token" ]]; then
        # Fallback jika BOT_TOKEN belum di-set sama sekali
        [[ ! -f "$BOTF" ]] && { err "Bot belum dikonfigurasi dan tidak ada token tersedia!"; return 1; }
        source "$BOTF" 2>/dev/null
        dl_token="${BOT_TOKEN:-}"
    fi
    if [[ -z "$dl_token" ]]; then
        err "Token bot kosong! Tidak bisa download."; return 1
    fi

    inf "Mendapatkan link download dari Telegram..."
    local file_info
    file_info=$(curl -s --max-time 30 \
        "https://api.telegram.org/bot${dl_token}/getFile?file_id=${file_id}" 2>/dev/null)

    if ! echo "$file_info" | grep -q '"ok":true'; then
        local tg_err; tg_err=$(echo "$file_info" | python3 -c \
            "import sys,json;d=json.load(sys.stdin);print(d.get('description','Unknown error'))" 2>/dev/null)
        err "getFile gagal: ${LR}${tg_err}${NC}"
        inf "Kemungkinan penyebab:"
        echo -e "  ${DIM}• File ID tidak valid atau sudah kedaluwarsa (>30 hari)${NC}"
        echo -e "  ${DIM}• Token bot tidak memiliki akses ke file ini${NC}"
        echo -e "  ${DIM}• File dikirim via bot berbeda — gunakan token bot yang sama${NC}"
        return 1
    fi
    ...
    if [[ -z "$file_path" ]]; then
        err "Gagal parse path file dari response Telegram!"
        return 1
    fi

    local dl_url="https://api.telegram.org/file/bot${dl_token}/${file_path}"
    inf "Mengunduh file backup... (tunggu sebentar)"

    if curl -s --max-time 300 -L "$dl_url" -o "$outfile" 2>/dev/null; then
        if [[ -s "$outfile" ]]; then
            # Verifikasi file adalah tar.gz yang valid
            if ! tar -tPf "$outfile" &>/dev/null; then
                err "File yang diunduh bukan backup valid (corrupt atau bukan tar.gz)!"
        ...
        err "Download gagal! Cek koneksi internet VPS."; rm -f "$outfile"; return 1
    fi
}
