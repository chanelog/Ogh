...
_bak_tg_list() {
    ...
    # Ambil update terbaru (max 100 pesan) untuk filter dokumen backup
    local updates
    ...
    if ! echo "$updates" | grep -q '"ok":true'; then
        err "Gagal mengambil data dari Telegram!"; return 1
    fi

    # Parse dokumen backup menggunakan python3
    local parsed
    parsed=$(python3 - "$CHAT_ID" <<'PYEOF'
import sys, json
...
svc_restore() {
    while true; do
        ...
        _sep
        _btn "  ${A2}[2]${NC}  📋  Restore dari Backup Lokal"
        _sep
        _btn "  ${A2}[3]${NC}  📁  Restore dari Path Manual"
        _sep
        _btn "  ${A2}[4]${NC}  🔍  Verifikasi / Lihat Isi Backup"
        _sep
        _btn "  ${LR}[0]${NC}  ◀   Kembali"
        _bot; echo ""
        echo -ne "  ${A1}›${NC} "; read -r ch
        ...
        1)
            ...
            # Ambil daftar backup dari Telegram
            if ! _bak_tg_list; then
                echo ""
                inf "Pastikan:"
                echo -e "  ${DIM}1. Backup pernah dikirim lewat menu Backup → [1]${NC}"
                echo -e "  ${DIM}2. Bot masih aktif dan token valid${NC}"
                pause; continue
            fi
        ...
        2)
            ...
            local cnt=$BAK_CNT
            if [[ "$cnt" == "0" ]]; then
                echo -e "\n  ${DIM}Gunakan pilihan [1] untuk restore dari Telegram.${NC}"
                pause; continue
            fi
